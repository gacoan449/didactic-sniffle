...
class WishlistService {
  ...
  Stream<List<WishlistModel>> daftarWishlist() {
    ...
    return ref.orderBy('ditambahkanPada', descending: true).snapshots().map((snap) {
      return snap.docs.map((d) => WishlistModel.dariMap(d.data(), d.id)).toList();
    });
  }
  ...
  Future<void> tambah(WishlistModel data) async {
    ...
    await ref.doc(data.produkId).set(data.keMap());
  }
