  ...
  dio.httpClientAdapter = IOHttpClientAdapter(
    createHttpClient: () {
      ...
      client.badCertificateCallback = (cert, host, port) {
        ...
        const daftarTerpercaya = ["hash_sertifikat_anda_1", "hash_sertifikat_anda_2"];
...
class PenyimpananAman {
  ...
  static Future<void> simpan(Map dariServer) async {
    await _penyimpananAman.write(key: _tokenAkses, value: dariServer["access_token"]);
    await _penyimpananAman.write(key: _tokenPerbarui, value: dariServer["refresh_token_baru"]);
    await _penyimpananAman.write(key: _kunciSesi, value: dariServer["session_key"]);
  }
...
class EnkripsiAman {
  static String? lindungi(String teks, String kunci) {
    try {
      ...
      final iv = Uint8List.fromList(List.generate(12, (_) => Random.secure().nextInt(255)));
...
class ModeDarurat {
  static Future<void> jalankan(String alasan) async {
    ...
    try { await buatKlienAman().post("/paksa-keluar", data: {"alasan": alasan}); } catch (_) {}
...
class PemeriksaBahaya {
  static Future<Map<String, dynamic>> cek() async {
    Map<String, dynamic> laporan = {"aman": true, "skor": 0, "alasan": <String>[]};
    List<String> jalur = ["/sbin/su", "/sbin/magisk", "/system/lib/libfrida.so"];
...
class PencatatAudit {
  static Future<void> catat(String aktivitas, Map detail) async {
    ...
    await buatKlienAman().post("/catat-audit", data: {
      "waktu": waktu, "aktivitas": aktivitas, "perangkat": sidik, "detail": detail
    });
  }
}
...
class PermintaanServer {
  static String buatNonce() => sha256.convert(utf8.encode(
  ...
  static Future<Map> kirim(String alamat, Map data, String kunci) async {
    ...
    final tanda = Hmac(sha256, utf8.encode(kunci))
      .convert(utf8.encode("${json.encode(data)}|$waktu|$nonce"))
      .toString();

    final hasil = await buatKlienAman().post(alamat, data: {
      "request_id": buatNonce(), "timestamp": waktu, "nonce": nonce,
      "signature": tanda, "app_version": "1.0.0", ...data
    });
...
void main() async {
  ...
  if(!cekAwal["aman"]) await ModeDarurat.jalankan(cekAwal["alasan"].join(", "));
          ...
          return Center(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Icon(laporan["aman"] ? Icons.verified : Icons.block,
                    size: 80, color: laporan["aman"] ? Colors.green : Colors.red),
                  SizedBox(height: 20),
                  Text(
                    laporan["aman"]
                  ...
                  SizedBox(height: 20),
                  Text("✅ IV Langsung Digabung", style: TextStyle(color: Colors.green, fontSize: 13)),
                  Text("✅ Hash APK Diverifikasi", style: TextStyle(color: Colors.green, fontSize: 13)),
                  Text("✅ Keputusan Penuh Server", style: TextStyle(color: Colors.green, fontSize: 13)),
                  Text("✅ Catatan Audit Langsung Terkirim", style: TextStyle(color: Colors.green, fontSize: 13)),
                ],
              ),
            ),
          );
